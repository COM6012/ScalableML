...
- [Task 1](#1-k-means-clustering): To finish in the lab session on 29th April. **Essential**
- [Task 2](#2-exercises): To finish by the following Monday 4th May. ***Exercise***
...
- [Clustering in Spark](https://spark.apache.org/docs/4.1.0/ml-clustering.html)
- [PySpark API on clustering](https://spark.apache.org/docs/4.1.0/api/python/reference/api/pyspark.ml.clustering.KMeans.html)
...
`KMeans` is implemented as an `Estimator` and generates a [`KMeansModel`](https://spark.apache.org/docs/4.1.0/api/python/reference/api/pyspark.ml.clustering.KMeansModel.html) as the base model.

[API](https://spark.apache.org/docs/4.1.0/api/python/reference/api/pyspark.ml.clustering.KMeans.html): `class pyspark.ml.clustering.KMeans(featuresCol='features', predictionCol='prediction', k=2, initMode='k-means||', initSteps=2, tol=0.0001, maxIter=20, seed=None, distanceMeasure='euclidean', weightCol=None)`
...
```sh
srun --account=rse-com6012 --reservation=rse-com6012-9 --cpus-per-task=2 --time=01:00:00 --pty /bin/bash
```
...
You can check out the [KMeansSummary API](https://spark.apache.org/docs/4.1.0/api/java/org/apache/spark/ml/clustering/KMeansSummary.html) for details of the summary information, e.g., we can find out that the training cost is the sum of squared distances to the nearest centroid for all points in the training dataset.

### Save and load an algorithm/model

We can save an algorithm/model in a temporary location (see [API on save](https://spark.apache.org/docs/4.1.0/api/python/reference/api/pyspark.ml.PipelineModel.html?highlight=pipelinemodel%20save#pyspark.ml.PipelineModel.save)) and then load it later.
